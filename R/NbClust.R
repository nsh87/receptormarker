...
NbClust <- function(data = NULL, diss = NULL, distance = "euclidean",
  ...
  Index.Hubert<-function(x, cl)
  {
    ...
    for (j in 1:n) variance.matrix[j] <- var(P[, j]) * (n - 1) / n
    ...
    meanQ <- mean(Q)
    for (j in 1:n) variance.matrix[j] <- var(Q[, j]) * (n - 1) / n
    ...
    for (i in 1:k) {
      ...
      if (sum(ind) > 1) {
        ...
        centerI <- matrix(centerI, nrow = sum(ind), ncol = ncol(x),
                          byrow = TRUE)
        S[i] <- mean(sqrt(apply((x[ind, ] - centerI) ^ 2, 1, sum)) ^ q) ^
          (1 / q)
      } else S[i] <- 0
    }
  ...
  Indice.Gap <- function(x, clall, reference.distribution = "unif", B = 10,
    ...
    GAP <- function(X, cl, referenceDistribution, B, method, d, centrotypes) {
      ...
      for (bb in (1:B)) {
        ...
        else stop("Wrong reference distribution type", call. = FALSE)
        if (bb == 1) {
          pp <- cl
  ...
  for (nc in min_nc:max_nc) {
    ...
    if (any((indice == 14) || (indice == 31) || (indice == 32))) {
      res[nc - min_nc + 1, 14] <- Indices.WKWL(x = jeu, cl1 = cl1,
                                               cl2 = cl2)[["duda"]]
    }
    ########### Indices.WKWL-pseudot2 - 15e colonne de res ############
    if (any((indice == 15) || (indice == 31) || (indice == 32))) {
      res[nc - min_nc + 1, 15] <- Indices.WKWL(x = jeu, cl1 = cl1,
                                               cl2 = cl2)[["pseudot2"]]
    }
