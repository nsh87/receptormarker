# NbClust Package for determining the best number of clusters Copyright (C) 2015
# Malika Charrad This program is free software; you can redistribute it and/or
...
#' @param diss dissimilarity matrix to be used. By default, diss=NULL, but if it
#'  is replaced by a dissimilarity matrix, distance should be "NULL".
#' @param distance the distance measure to be used to compute the dissimilarity
#'   matrix. This must be one of: "euclidean", "maximum", "manhattan",
#'   "canberra", "binary", "minkowski" or "NULL". By default,
#'   distance="euclidean". If the distance is "NULL", the dissimilarity matrix
#'   (diss) should be given by the user. If distance is not "NULL", the
#'   dissimilarity matrix should be "NULL".
#' @param min.nc minimal number of clusters, between 1 and (number of objects
#'   - 1)
#' @param max.nc maximal number of clusters, between 2 and (number of objects
#'   - 1), greater or equal to min.nc. By default, max.nc=15.
#' @param method the cluster analysis method to be used. This should be one of:
#'   "ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median",
#'   "centroid", "kmeans".
#' @param index the index to be calculated. This should be one of : "kl", "ch",
...
#'   "gplus", "tau", "dunn", "hubert", "sdindex", "dindex", "sdbw", "all" (all
#'   indices except GAP, Gamma, Gplus and Tau), "alllong" (all indices with Gap,
...
NbClust <- function(data = NULL, diss = NULL, distance = "euclidean",
  ...
  Dis <- function(cl, x) {
    ...
    k <- max(cl)
    centers.matrix <- centers(cl, x)
    Distance.centers <- as.matrix(dist(centers.matrix))
    Dmin <- min(Distance.centers)
    Dmax <- max(Distance.centers)
    s2 <- 0
    for (u in 1:k) {
      s2 <- s2 + 1 / sum(Distance.centers[u, ])
    }
    Dis <- Dmax / Dmin * s2
    return(Dis)
  }
  ...
  Indice.ptbiserial <- function(x, md, cl1) {
    ...
    biserial.cor <- function(x, y, use = c("all.obs", "complete.obs"),
      ...
      if (length(x) != length(y)) 
        stop("'x' and 'y' do not have the same length", call. = FALSE)
      use <- match.arg(use)
      if (use == "complete.obs") {
  ...
  Indice.Gap <- function(x, clall, reference.distribution = "unif", B = 10,
    ...
    X <- as.matrix(x)
    gap1 <- GAP(X, clall[, 1], reference.distribution, B, method, d,
                centrotypes)
    gap <- gap1[["Sgap"]]
  ...
  for (nc in min_nc:max_nc) {
    ...
    if (indice < 31) {
      if (indice == 3) {
        res[nc - min_nc + 1, 3] <- Indices.Traces(jeu, md, clall1,
          index = "hart")
      } else if (indice == 4) {
        res[nc - min_nc + 1, 4] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["ccc"]]
      } else if (indice == 5) {
        res[nc - min_nc + 1, 5] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["scott"]]
      } else if (indice == 6) {
        res[nc - min_nc + 1, 6] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["marriot"]]
      } else if (indice == 7) {
        res[nc - min_nc + 1, 7] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["trcovw"]]
      } else if (indice == 8) {
        res[nc - min_nc + 1, 8] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["tracew"]]
      } else if (indice == 9) {
        res[nc - min_nc + 1, 9] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["friedman"]]
      } else if (indice == 10) {
        res[nc - min_nc + 1, 10] <- Indices.WBT(x = jeu, cl = cl1, P = TT,
          s = ss, vv = vv)[["rubin"]]
      } else if (any(indice == c(14, 15, 16))) {
        WKWL <- Indices.WKWL(x = jeu, cl1 = cl1, cl2 = cl2)
        NM <- WKWL[["NM"]]
        NK <- WKWL[["NK"]]
        NL <- WKWL[["NL"]]
        zz <- 3.2  # Best standard score in Milligan and Cooper 1985
        zzz <- zz * sqrt(2 * (1 - 8 / (pi ^ 2 * pp)) / (NM * pp))
        if (indice == 14) {
          res[nc - min_nc + 1, 14] <- WKWL[["duda"]]
          resCritical[nc - min_nc + 1, 1] <- critValue <- 1 - 2 / (pi * pp) -
            zzz
        } else if (indice == 15) {
          res[nc - min_nc + 1, 15] <- WKWL[["pseudot2"]]
          critValue <- 1 - 2 / (pi * pp) - zzz
          resCritical[nc - min_nc + 1, 2] <- (1 - critValue) / critValue *
            (NK + NL - 2)
        } else if (indice == 16) {
          res[nc - min_nc + 1, 16] <- beale <- WKWL[["beale"]]
          df2 <- (NM - 2) * pp
          resCritical[nc - min_nc + 1, 3] <- 1 - pf(beale, pp, df2)
        }
      } else if (indice == 18) {
        res[nc - min_nc + 1, 18] <- Indices.Traces(jeu, md, clall1,
          index = "ball")
      } else if (indice == 19) {
        res[nc - min_nc + 1, 19] <- Indice.ptbiserial(x = jeu, md = md,
          cl1 = cl1)
      } else if (indice == 20) {
        if (methodM == 4) {
      ...
      res[nc - min_nc + 1, 4] <- WBT[["ccc"]]
      res[nc - min_nc + 1, 5] <- WBT[["scott"]]
      res[nc - min_nc + 1, 6] <- WBT[["marriot"]]
      res[nc - min_nc + 1, 7] <- WBT[["trcovw"]]
      res[nc - min_nc + 1, 8] <- WBT[["tracew"]]
      res[nc - min_nc + 1, 9] <- WBT[["friedman"]]
      res[nc - min_nc + 1, 10] <- WBT[["rubin"]]
      WKWL <- Indices.WKWL(x = jeu, cl1 = cl1, cl2 = cl2)
      NM <- WKWL[["NM"]]
      NK <- WKWL[["NK"]]
      NL <- WKWL[["NL"]]
    ...
    if (nc >= 2) {
      if (indice < 31) {
        if (indice == 1) {
          res[nc - min_nc + 1, 1] <- Indices.Traces(jeu, md, clall1,
            index = "kl")
        } else if (indice == 2) {
          res[nc - min_nc + 1, 2] <- Indices.Traces(jeu, md, clall1,
            index = "ch")
        } else if (indice == 11) {
          ...
          res[nc - min_nc + 1, 17] <- Indices.Traces(jeu, md, clall1,
            index = "ratkowsky")
        } else if (indice == 21) {
          res[nc - min_nc + 1, 21] <- Index.15and28(cl1 = cl1, cl2 = cl2,
            md = md)[["frey"]]
        } else if (indice == 22) {
          res[nc - min_nc + 1, 22] <- Index.15and28(cl1 = cl1, cl2 = cl2,
            md = md)[["mcclain"]]
        } else if (indice == 23) {
          ...
          res[nc - min_nc + 1, 26] <- Index.dunn(md, cl1, Data = jeu,
            method = NULL)
        } else if (indice == 27) {
        ...
        res[nc - min_nc + 1, 17] <- Indices.Traces(jeu, md, clall1,
          index = "ratkowsky")
        and1528 <- Index.15and28(cl1 = cl1, cl2 = cl2, md = md)
        res[nc - min_nc + 1, 21] <- and1528[["frey"]]
        res[nc - min_nc + 1, 22] <- and1528[["mcclain"]]
        res[nc - min_nc + 1, 26] <- Index.dunn(md, cl1, Data = jeu,
          method = NULL)
        res[nc - min_nc + 1, 27] <- Index.Hubert(jeu, cl1)
        res[nc - min_nc + 1, 28] <- Index.sdindex(jeu, clmax, cl1)
        ...
        if (indice == 32) {
          ...
          res[nc - min_nc + 1, 23] <- sPlussMoins[["gamma"]]
          res[nc - min_nc + 1, 24] <- sPlussMoins[["gplus"]]
          res[nc - min_nc + 1, 25] <- sPlussMoins[["tau"]]
        }
      }
    } else {
  ...
  # Some indices need to compute difference between hierarchy levels to identify
  # relevant number of clusters
  if (any((indice == 3) || (indice == 5) || (indice == 6) || (indice == 7) ||
          (indice == 8) || (indice == 9) || (indice == 10) || (indice == 18) ||
          (indice == 27) || (indice == 11) || (indice == 29) ||
          (indice == 31) || (indice == 32))) {
    DiffLev <- array(0, c(max_nc - min_nc + 1, 12))
    DiffLev[, 1] <- min_nc:max_nc
    for (nc3 in min_nc:max_nc) {
      if (nc3 == min_nc) {
        DiffLev[nc3 - min_nc + 1, 2] <- abs(res[nc3 - min_nc + 1, 3] - NA)
        DiffLev[nc3 - min_nc + 1, 3] <- abs(res[nc3 - min_nc + 1, 5] - NA)
        DiffLev[nc3 - min_nc + 1, 4] <- abs(res[nc3 - min_nc + 1, 6] - NA)
        DiffLev[nc3 - min_nc + 1, 5] <- abs(res[nc3 - min_nc + 1, 7] - NA)
        DiffLev[nc3 - min_nc + 1, 6] <- abs(res[nc3 - min_nc + 1, 8] - NA)
        DiffLev[nc3 - min_nc + 1, 7] <- abs(res[nc3 - min_nc + 1, 9] - NA)
  ...
  if (any((indice == 27) || (indice == 31) || (indice == 32)) && plots) {
    ...
    message("*** : The Hubert index is a graphical method of determining the ",
  ...
  if (any((indice == 29) || (indice == 31) || (indice == 32)) && plots) {
    ...
    message(paste("*** : The D index is a graphical method of determining the ",
                  "number of clusters. \n                   In the plot of D ",
  ...
  if (any((indice == 20) || (indice == 23) || (indice == 24) ||
    ...
    results <- c(nc.KL, indice.KL, nc.CH, indice.CH, nc.Hartigan,
                 ...
                 nc.Rubin, indice.Rubin, nc.cindex, indice.cindex, nc.DB,
                 indice.DB, nc.Silhouette, indice.Silhouette, nc.Duda,
                 indice.Duda, nc.Pseudo, indice.Pseudo, nc.Beale, indice.Beale,
                 nc.Ratkowsky, indice.Ratkowsky, nc.Ball, indice.Ball,
                 nc.ptbiserial, indice.ptbiserial, nc.Gap, indice.Gap, nc.Frey,
                 indice.Frey, nc.McClain, indice.McClain, nc.Gamma,
                 indice.Gamma, nc.Gplus, indice.Gplus, nc.Tau, indice.Tau,
                 nc.Dunn, indice.Dunn, nc.Hubert, indice.Hubert, nc.sdindex, 
    ...
    results <- c(nc.KL, indice.KL, nc.CH, indice.CH, nc.Hartigan,
                 ...
                 indice.Duda, nc.Pseudo, indice.Pseudo, nc.Beale, indice.Beale,
                 nc.Ratkowsky, indice.Ratkowsky, nc.Ball, indice.Ball,
                 nc.ptbiserial, indice.ptbiserial, nc.Frey, indice.Frey,
                 nc.McClain, indice.McClain, nc.Dunn, indice.Dunn, nc.Hubert,
                 indice.Hubert, nc.sdindex, indice.sdindex, nc.Dindex,
